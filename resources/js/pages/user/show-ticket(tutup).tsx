...
const statusDotClass = (s: TicketStatus) =>
  s === "baru" ? "bg-amber-500" : s === "proses" ? "bg-blue-500" : s === "ditahan" ? "bg-rose-500" : "bg-red-500";
...
const ShowTicket: React.FC = () => {
  ...
  const steps = [
    ...
    { key: "confirmation",label: "Menunggu Konfirmasi Pengguna",   note: "Diterima/Ditolak", time: "02/11/2025 13:45", state: "done",    icon: "fa-check-circle" },
  ] as Array<{ key: string; label: string; note?: string; time?: string; state: "done" | "current" | "todo"; icon: string }>;

  const isClosed = ticket.status === "ditutup";
  ...
  const { post: postRating, processing: savingRating } = useForm<RatingPayload>({
    ticket_id: ticket.id,
    rating,
    note: ratingNote,
  });
  ...
  const submitRating = (e: React.FormEvent) => {
    e.preventDefault();
    postRating("/tickets/rating", {
      data: { ticket_id: ticket.id, rating, note: ratingNote },
    } as any);
  };

  const shouldShowFullReplyForm = !isClosed && messages.length === 0;
        ...
        <div className={`${container} py-8`}>
          <div className="grid grid-cols-1 gap-6 md:grid-cols-12">
            ...
            <aside className="md:col-span-4 space-y-6">
              ...
              {/* Rating (muncul hanya saat ditutup) */}
              {isClosed && (
                <div className="rounded-xl bg-white ring-1 ring-gray-100 shadow-sm p-6">
                  <h3 className="text-sm font-semibold text-gray-800">Rating Kepuasan Pelayanan</h3>
                  <form onSubmit={submitRating} className="mt-3">
                    <div className="flex items-center gap-2">
                      {[1,2,3,4,5].map((n) => (
                        ...
                        >
                          <i className={`fas fa-star ${ (hover || rating) >= n ? "text-yellow-400" : "text-gray-300" }`} />
                        </button>
                      ))}
                      <span className="ml-2 text-sm text-gray-600">{rating > 0 ? `${rating}/5` : "Belum dipilih"}</span>
                    </div>
                    <textarea
                      className="mt-3 w-full rounded-lg border border-gray-200 px-3 py-2 text-sm outline-none focus:border-[#2558B0] focus:ring-2 focus:ring-[#2558B0]/15"
                    ...
                    <button
                      type="submit"
                      disabled={rating === 0}
                      className="mt-3 inline-flex items-center gap-2 rounded-lg bg-[#4C7CF6] px-4 py-2 text-sm font-semibold text-white shadow hover:bg-[#3f6be0] disabled:opacity-60"
                    >
                      Kirim Rating
                    </button>
                  </form>
            ...
            <section className="md:col-span-8 space-y-6">
              ...
              {isClosed && (
                <div className="rounded-lg bg-amber-100 text-amber-900 ring-1 ring-amber-200 px-4 py-2">
                  <i className="fas fa-lock mr-2" />
                  <span className="font-semibold">Tiket ini sudah ditutup.</span> Anda tidak dapat membalas tiket lagi.
                </div>
              )}
                      ...
                      <form onSubmit={onSubmit}>
                        ...
                        <div className="mt-6">
                          ...
                          <div className="mt-2 grid grid-cols-2 gap-3 text-sm sm:flex sm:items-center sm:gap-6">
                            ...
                            <label className="inline-flex items-center gap-2">
                              <input type="radio" name="status" value="proses" checked={data.status === "proses"} onChange={() => setData("status", "proses")} className="h-4 w-4 accent-[#2558B0]" />
                              <span>Proses</span>
                            </label>
                            <label className="inline-flex items-center gap-2">
                              <input type="radio" name="status" value="ditahan" checked={data.status === "ditahan"} onChange={() => setData("status", "ditahan")} className="h-4 w-4 accent-[#2558B0]" />
                              <span>Ditahan</span>
                            </label>
                            <label className="inline-flex items-center gap-2">
